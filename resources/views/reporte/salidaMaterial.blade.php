  ...
  <div id="main-container">
    <div class="flex-container" style="float:left">
      ...
      <table align="center"  width="100%">
        <thead>
          <tr align="center">
              <td colspan="" style="font-size: 120%; font-weight: bold;">SALIDA DE MATERIALES ( {{$fecha1}} / {{$fecha2}})</strong></td>
          </tr>
        </thead>
      </table>   
      <br>
      ...
      <table align="center"  border="1" >
        <thead>
          ...
          <tr style="background-color: #457089; color: black;font-weight: bold; font-size: 15px;" class='content' id="cabeza">
            ...
            <th style="font-size: 85%;" align="center">PERSONAL</th>
        ...
        <tbody>
            @php($count=1)
            @foreach ($salidaMateriales as  $value)
                <tr class='content'>
                    <td style="font-size: 80%;">{{$count++}}</td>
                    <td style="font-size: 80%;">{{$value->nombreMaterial}}</td>
                    <td style="font-size: 80%;">{{$value->grado}} {{$value->nombre}} {{$value->apellidos}}</td>
                    <td style="font-size: 80%;">{{$value->cargo}}</td>
