...
export default function CategoriaProductoEdit({errors,categoriaProducto,flash,permisos }) {
    const { data, setData, post,put } = useForm({
        categoria: categoriaProducto.categoria,
        descripcion: categoriaProducto.descripcion,
        estado: categoriaProducto.estado,
    });
    ...
    function handleSubmit(e) {
        e.preventDefault();
        put(route("categoriaProducto.update",{ id: categoriaProducto.id }));
    }
                                ...
                                <form onSubmit={handleSubmit} className="p-fluid">
                                    <div className="-mx-3 md:flex mb-6">
                                        ...
                                        <label className="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2" for="grid-password">
                                           CATEGORIA
                                            ...
                                            <InputText id="categoria" name="categoria" value={data.categoria} onChange={(e) => setData("categoria", e.target.value.toUpperCase())} required/>
                                    ...
                                    <div className="p-field mb-4 flex justify-between">
                                        ...
                                        <Button type="submit" label="Actualizar" className="p-button-sm mr-2" severity="info" outlined  />
                                            <Link className="p-button p-button-sm p-button-danger" href={route("categoriaProducto.index")} style={{color:'white'}}>Cancelar</Link>
                                        </div>
                                    </div>
                                </form>
                            </div>
