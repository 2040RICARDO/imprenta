...
export default function SalidaMaterialCreate({errors,salidaMaterial,materiales,personales,material,personal,permisos }) {



    const { data, setData, put } = useForm({
        descripcion: salidaMaterial.descripcion,
        cantidad: salidaMaterial.cantidad,
        materialId: salidaMaterial.material_id,
        personalId: salidaMaterial.personal_id,

        descripcionMaterial:material.descripcion,
        cantidadDisponible: parseInt(material.stock)+parseInt(salidaMaterial.cantidad),
        cargo:personal.cargo,
        disabledInput:false,

       /*  materialIdFijo:salidaMaterial.material_id,
        cantidadDisponibleFijo:salidaMaterial.cantidad
 */
    });
    const materialIdFijo =salidaMaterial.material_id;
    const cantidadDisponibleFijo =salidaMaterial.cantidad;
    console.log(data.cantidadDisponible);

    function handleSubmit(e) {
        e.preventDefault();
        put(route("salidaMaterial.update",{ id: salidaMaterial.id }));
    }
                                ...
                                <form onSubmit={handleSubmit} className="p-fluid">
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (selectedMaterial) {
                                                        ...
                                                        setData({
                                                            ...
                                                            cantidadDisponible:materialIdFijo== selectedMaterial.id ?(parseInt(selectedMaterial.stock) + parseInt(cantidadDisponibleFijo)) :selectedMaterial.stock,
                                                            disabledInput:materialIdFijo == e.target.value?( parseInt(selectedMaterial.stock) + parseInt(cantidadDisponibleFijo) != 0?false:true):(selectedMaterial.stock != 0?false:true),
                                                            cargo:data.cargo,
                                    ...
                                    <div className="p-field mb-4 flex justify-between">
                                        <div className="flex">
                                            ...
                                            <Link className="p-button p-button-sm p-button-danger" href={route("salidaMaterial.index")} style={{color:'white'}}>Cancelar</Link>
                                        </div>
                                    </div>
                                </form>
                            </div>
